...
from model.mvn.utils import op, multiview, img, misc, volumetric
from model.mvn.models import pose_resnet
from model.mvn.models.v2v import V2VModel
...
class RANSACTriangulationNet(nn.Module):
    ...
    def forward(self, images, proj_matricies, batch):
        ...
        keypoints_2d = torch.stack([max_indicies % heatmap_shape[1], max_indicies // heatmap_shape[1]], dim=-1).to(
            images.device)
        ...
        for batch_i in range(batch_size):
            for joint_i in range(n_joints):
                ...
                keypoint_3d, _ = self.triangulate_ransac(current_proj_matricies, points,
                                                         direct_optimization=self.direct_optimization)
    ...
    def triangulate_ransac(self, proj_matricies, points, n_iters=10, reprojection_error_epsilon=15,
        ...
        for i in range(n_iters):
            ...
            multiview.calc_reprojection_error_matrix(np.array([keypoint_3d_in_base_camera]), points, proj_matricies)[0]
        ...
        keypoint_3d_in_base_camera = multiview.triangulate_point_from_multiple_views_linear(inlier_proj_matricies,
                                                                                            inlier_points)
        reprojection_error_vector = \
        multiview.calc_reprojection_error_matrix(np.array([keypoint_3d_in_base_camera]), inlier_points,
        ...
        if direct_optimization:
            def residual_function(x):
                reprojection_error_vector = \
                multiview.calc_reprojection_error_matrix(np.array([x]), inlier_points, inlier_proj_matricies)[0]
