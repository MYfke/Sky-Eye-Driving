...
from model.mvn.utils.img import image_batch_to_torch
...
def make_collate_fn(randomize_n_views=True, min_n_views=4, max_n_views=31):
    def collate_fn(items):
        ...
        batch['images'] = np.stack([np.stack([item['images'][i] for item in items], axis=0) for i in indexes],
                                   axis=0).swapaxes(0, 1)
...
def prepare_batch(batch, device, config, is_train=True):
    ...
    # 3D keypoints validity  3D 关键点有效性
    keypoints_3d_validity_batch_gt = torch.from_numpy(
        np.pad(np.stack(batch['keypoints_3d'], axis=0), ((0, 0), (0, 0),(0, 1)), constant_values=1)[:, :,
        3:]).float().to(
        device)  # TODO 将最后一维填充为1，现在写法比较丑陋

    # projection matricies  投影矩阵
    proj_matricies_batch = torch.stack(
        [torch.stack([torch.from_numpy(camera.projection) for camera in camera_batch], dim=0) for camera_batch in
