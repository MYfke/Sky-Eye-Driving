"""
    ...
    为多视图“human36m.py”生成“labels.npy”
...
# 如果您想使用 Mask-RCNN 或 SSD 边界框而不是 H36M 的“ground truth”，请更改此行。
h36m_root = "data/human36m/processed/"
labels_path = "data/human36m/extra/human36m-multiview-labels-GTbboxes.npy"
una_dinosauria_root = "./data/human36m/extra/una-dinosauria-data"
...
retval['cameras'] = np.zeros(
    (len(retval['subject_names']), len(retval['camera_names'])),  # 每个相机的参数
    dtype=[
        ('R', np.float32, (3, 3)),
        ('t', np.float32, (3, 1)),
        ('K', np.float32, (3, 3)),
...
table_dtype = np.dtype([
    ...
    ('action_idx', np.int8),
    ('frame_idx', np.int16),
    ('keypoints', np.float32, (17, 3)),  # roughly MPII format  大致 MPII 格式
...
# una_dinosauria_root = r"D:\Projects\learnable-triangulation-pytorch-master\data\human36m\extra\una-dinosauria-data\cameras.h5"
cameras_params = h5py.File(os.path.join(una_dinosauria_root, 'cameras.h5'), 'r')
# cameras_params = h5py.File(una_dinosauria_root, 'r')

# Fill retval['cameras']  填充 retval['cameras']
...
for subject_idx, subject in enumerate(retval['subject_names']):
    ...
    for action_idx, action in enumerate(retval['action_names']):
        ...
        with h5py.File(os.path.join(una_dinosauria_root, subject, 'MyPoses', '3D_positions',
                                    '%s.h5' % action_to_una_dinosauria[subject].get(action, action.replace('-', ' '))),
                       'r') as poses_file:
