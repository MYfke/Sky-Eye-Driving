...
class BITMAPFILEHEADER(Structure):  # 位图文件头
    _fields_ = [
        ('bfType', c_ushort),
        ('bfSize', c_uint),
...
def demo():
    ...
    memmove(frameBuff, c_char_p(buffAddr), imageSize)  # 将图片数据传给frameBuff
    ...
    convertParams.timeStamp = frame.contents.getImageTimeStamp(frame)
    ...
    if convertParams.pixelForamt == EPixelType.gvspPixelMono8:
        ...
        nRet = IMGCNV_ConvertToBGR24(cast(frameBuff, c_void_p), byref(convertParams),
                                     cast(rgbBuff, c_void_p), byref(rgbSize))  # 这里rgbbuff被写入
    ...
    imageFile = open(fileName, 'wb+')  # 从此处开始写
    ...
    if convertParams.pixelForamt == EPixelType.gvspPixelMono8:
        ...
        imageFile.writelines(rgbBuff)  # 此处写入照片
