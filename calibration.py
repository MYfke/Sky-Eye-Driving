...
def onGetFrameEx(frame, userInfo):
    ...
    '''
    cv2.imshow(f'{c_char_p(userInfo).value}', convert(frame))
    gc.collect()
    cv2.waitKey(1)
...
def main():
    ...
    choice = eval(input("输入要选择的相机ID:"))

    for index in range(choice, choice + 2):
        # 设置软触发
        # set software trigger config
        nRet = setSoftTriggerConf(cameraList[index])
        if nRet != 0:
            ...
            print(f"camera {index} set SoftTriggerConf success!")
        ...
        # TODO 这句代码厉害了，源代码就是因为有这句才不报错
        time.sleep(1)

    while True:
        key = input('按下回车继续，按下q结束')
        if key != 'q':
            for index in range(choice, choice + 2):
                # 软触发取一张图
                # execute software trigger to grab one frame
                nRet = grabOne(cameraList[index])
                if nRet != 0:
                    print("grabOne fail!")
                    # 释放相关资源
                    # release stream source object before return
                ...
                nRet = streamSourceList[index].contents.getFrame(streamSourceList[index], byref(frame), c_uint(1000))
                if nRet != 0:
                ...
                cv2.imwrite(f'./data/calibration_image/C{choice}/{str(frame.contents.getBlockId(frame))}.jpg',
                            convert(frame))
        else:
            break
    ...
    for index in range(choice, choice + 1):
        # 关闭相机
        # close camera
        nRet = closeCamera(cameraList[index])
        if nRet != 0:
